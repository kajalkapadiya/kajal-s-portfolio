...
const Skill = ({ name, x, y }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }} // only for once it animate.
    >
      {name}
    </motion.div>
  );
...
const Skills = () => {
  return (
    <>
      <h2 className="font-bold text-8xl mt-40 md:text-6xl md:mt-28 w-full text-center">skills</h2>
      <div className="w-full h-screen relative flex items-center justify-center rounded-full bg-circularLight 
      dark:bg-circularDark lg:h-[80vh] sm:h-[60vh] xs:h-[50vh] lg:bg-circularLightLg lg:dark:bg-circularDarkLg sm:bg-circularLightSm sm:dark:bg-circularDarkSm md:bg-circularLightMd md:dark:bg-circularDarkMd">
        <motion.div
          className="flex items-center justify-center rounded-full font-semibold
